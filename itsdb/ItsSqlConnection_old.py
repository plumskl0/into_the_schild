...
class ItsSqlConnection():
    ...
    def createDefaultDatabase(self,):
        self.__debug('Database does not exist. Creating database \'{}\''.format(sql_cfg.database))
        self.its_cur.execute('CREATE DATABASE {}'.format(self.sql_cfg.database))
        self.its_cur.execute('USE {}'.format*self.sql_cfg.database)
    ...
    def getImageFromRequestHistory(self, reqHistoryIds):
        ...
        stmt = 'SELECT img_blob FROM its_request_history WHERE id IN ({})'.format(
            ids)
