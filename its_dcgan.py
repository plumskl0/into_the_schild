...
from istlogging import ITSLogger
...
class ITSdcgan():
    ...
    def __init__(self):
        ...
        self.log = ITSLogger(logName=logName, debug=debug)

        # Ordner
        self.root = './'
        self.dirItsImages = os.path.join(root, 'its_images')
        self.dirRunImages = createRunFolderName()
        ...
        self.images, self.labels = generateData()
        self.imgShape = [None, 64, 64, 3]


    def initEpoch(self, epochs, n_noise):
        epochs = 1
        n_noise = 64
        index_in_epoch = 0
        epochs_completed = 0
    ...
    def generateData(self):
        ...
        log.info('Found {} files in directory {}'.format(
            len(files), dirItsImages))
        ...
        imgs = np.multiply(images, 1.0 / 255.0)
    ...
    def discriminator(self, img_in, reuse=None, keep_prob=keep_prob):
        activation = lrelu
    ...
    def generator(self, z, keep_prob=keep_prob, is_training=is_training):
        activation = lrelu
        momentum = 0.99
